...
export default function LeaveOfAbsencePage() {
  ...
  const earlyReturnMut = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["/api/approvals"] });
      toast({ title: "Early return submitted", description: "Pending admin approval." });
      setEarlyReturnDate("");
    ...
    <div className="p-3 md:p-4 tac-page max-w-3xl">
      ...
      {pendingEarly ? (
        <div className="rounded-md border border-amber-900/50 bg-amber-950/20 px-3 py-2 mb-4 text-[11px] text-amber-200/90">
          <div className="font-bold tracking-wider">EARLY RETURN PENDING APPROVAL</div>
      ...
      {activeLoa && canRequestEarly ? (
        <div className="rounded-md border border-border bg-card p-4 mb-6 space-y-3">
          <div className="text-[10px] text-muted-foreground tracking-wider uppercase flex items-center gap-2">
            <LogIn className="h-3.5 w-3.5" /> Come back early
          </div>
          <p className="text-[10px] text-muted-foreground">
            Request a new return date <span className="text-foreground">before</span> your scheduled end ({fmtDate(activeLoa.end)}
            ). Admins approve in the Approvals queue.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label className="text-[10px] tracking-wider">Return on (YYYY-MM-DD)</Label>
              <Input
                className="text-xs"
                type="date"
                min={todayStr}
